...
- [x] Split `BasicMapScreen.tsx`, `DailyFilterModal.tsx` (mobile — on-device verified) — Phase 9 (RFCT-02/RFCT-03); `filterStore.ts` ✓ Phase 8 (RFCT-01)
- [x] Split `ReviewDishEditor.tsx` (admin — operator in-browser Save verified) — Phase 10 (RFCT-04)
...
---
*Last updated: 2026-06-24 after Phase 10 (Admin Editor Refactor) completion — RFCT-04 validated: `ReviewDishEditor.tsx` (1258 lines) decomposed into a `ReviewDishEditor/` directory (orchestration-root+barrel `index.tsx`, pure `reviewHelpers.ts`/`buildConfirmPayload.ts`, presentational `BundledItemsBlock`/`CategorySection`/`DishCard`), split along form regions with a single `buildConfirmPayload()`+submit keeping the `admin_confirm_menu_scan` payload byte-identical (inline-snapshot + adversarial review diff), 5 landmines preserved, `turbo check-types` green, admin unit suite 169/169, code review clean (0 critical/0 warning), phase verification 3/3, operator in-browser Save approved. The refactor track (RFCT-01..04) is now complete. Open milestone items: Phase 6 (Schema Teardown Spine) and the Performance & scaling track remain; one on-device UI check (CLEAN-01) still pending in 05-HUMAN-UAT.md; the admin-confirm-rpc integration test is env-gated (needs a migrated local Supabase — `supabase db reset`).*
