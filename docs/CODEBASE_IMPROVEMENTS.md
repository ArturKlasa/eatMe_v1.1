...
| ---- | -------- | ---------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| N1.1 | ✅       | `apps/web-portal/lib/export.ts`                      | **Documented.** Added a top-of-file notice explaining the functions exist and are complete but not yet wired to any UI. Retained the file as the foundation for a future dashboard Export feature.                        |
| N1.2 | ✅       | `packages/database/src/client.ts` → `getWebClient()` | **Marked `@deprecated`.** Added JSDoc explaining the web portal now uses `createBrowserClient` from `@supabase/ssr`; updated `index.ts` module comment accordingly. Function retained to avoid a breaking package change. |
| N1.3 | ✅       | `packages/database/src/repositories/`                | **Deleted.** Empty directory removed.                                                                                                                                                                                     |
| N1.4 | ✅       | `apps/mobile/src/data/`                              | **Deleted.** Empty directory removed.                                                                                                                                                                                     |
| N1.5 | ✅       | `apps/web-portal/app/onboard/page.tsx`               | **Fixed.** Redirect now goes to `/onboard/basic-info` (the correct step 1). Stale comment removed.                                                                                                                        |
| N1.6 | ✅       | `apps/mobile/src/hooks/useRestaurants.ts`            | **Deleted.** Confirmed zero callers outside the hook itself. File removed; export removed from `hooks/index.ts`.                                                                                                          |
| N1.7 | ✅       | `apps/mobile/src/screens/BasicMapScreen.tsx`         | **Deleted.** `parseLocation()` had zero call sites; removed the entire function definition (~35 lines).                                                                                                                   |
| N1.8 | ✅       | `apps/web-portal/app/onboard/review/page.tsx`        | **Fixed.** Removed `handleEditRestaurantInfo`; the Restaurant Information "Edit" button now calls `handleBack` directly.                                                                                                  |
