...
**Status:** Shipped 2026-06-01. Swipe-feed follow-up shipped 2026-06-03 (migration 150).
...
- Follow-up: `feat(feed): surface currency_code through generate_candidates → swipe feed` — migration 150 adds `restaurant_currency_code` to the RPC return shape; feed Edge Function threads it into the dish payload; `MapFooter.tsx` renders via `formatPrice`. Closes the previous USD-fallback interim state.
