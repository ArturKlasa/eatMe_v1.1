-- 111_primary_protein_user_prefs_and_feed.sql
-- Created: 2026-04-20
--
-- 1. Add primary_protein column to user_preferences so the preference survives
--    cross-device sessions (persisted alongside other permanent filter fields).
-- 2. Update generate_candidates() RETURNS TABLE to expose primary_protein from
--    dishes, allowing the feed edge function to score against it.

-- ── 1. user_preferences: add primary_protein ──────────────────────────────────

ALTER TABLE public.user_preferences
  ADD COLUMN IF NOT EXISTS primary_protein text
  CHECK (
...
COMMENT ON COLUMN public.user_preferences.primary_protein IS
  'User''s preferred primary protein. Matches dishes.primary_protein enum. NULL = no preference.';

-- ── 2. generate_candidates(): add primary_protein to RETURNS TABLE ────────────
--
-- CREATE OR REPLACE cannot change the return type, so we DROP first.
-- Parameter types must exactly match the current signature (from migration 073).
