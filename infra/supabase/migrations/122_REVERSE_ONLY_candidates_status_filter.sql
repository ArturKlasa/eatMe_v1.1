-- 122_REVERSE_ONLY_candidates_status_filter.sql
-- Reverse migration for 122_candidates_status_filter.sql
--
-- Restores both RPCs to their pre-122 bodies (verbatim from migrations 088 + 114).
-- Safe to run as long as no code hard-depends on draft rows being excluded;
-- consumers will see all active rows regardless of status after rollback.

BEGIN;

-- Restore generate_candidates to migration 114 body (no status filters).
...
-- Restore get_group_candidates to migration 088 body (no status filters).
