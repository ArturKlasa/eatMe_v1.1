-- 129_fix_get_admin_restaurants_email_type.sql
--
-- Fix get_admin_restaurants RPC: auth.users.email is varchar(255), so
-- COALESCE(u.email, '') returns varchar and Postgres rejects the result with
-- 42804 ("Returned type character varying does not match expected type text in
-- column 7"). Add explicit ::text cast.
--
-- Originally introduced in 116c_restaurants_name_trgm.sql.
-- Reverse: 129_REVERSE_ONLY_fix_get_admin_restaurants_email_type.sql
