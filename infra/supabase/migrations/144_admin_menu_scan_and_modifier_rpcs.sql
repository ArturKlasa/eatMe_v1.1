-- 144_admin_menu_scan_and_modifier_rpcs.sql
-- Created: 2026-05-19
--
-- Phase 4.2 of the dish-model rewrite (docs/plans/dish-model-rewrite-phase-4-admin.md §2).
--
-- Introduces two admin-only RPCs that wrap multi-step write workflows in a
-- single Postgres transaction, eliminating the orphan-row class of bugs that
-- plagued the previous JS-side multi-pass inserts (parent → variants → courses
-- → standalones).
--
--   1. admin_confirm_menu_scan(job_id, admin_id, payload)
--      Persists a reviewed menu scan: menu_categories upserts + dishes (parents
--      / variants / courses) + modifier groups + audit log + job state
--      transition. Accepts BOTH the legacy (variant_dishes + courses) and the
--      new (modifier_groups) per-dish payload shapes during the Phase 4.2→4.3
--      transition window.
--
--   2. admin_replace_dish_modifiers(dish_id, groups)
--      Atomically replaces a dish's full modifier_groups set: deletes existing
--      option_groups (cascades to options) and inserts the supplied groups +
--      options. Used by the restaurant-detail dish editor in Phase 4.4.
--
-- Auth model (matches plan §2):
--   SECURITY INVOKER (default). The service-role caller in the admin app
--   (`createAdminServiceClient()`) bypasses RLS. The withAdminAuth wrapper is
--   the only authorization gate; the GRANT below limits the function to the
--   service_role to keep the perimeter tight.
--
-- Error contract:
--   On expected-state failure (job already completed, restaurant missing, etc.)
--   the function raises an exception with a STABLE MESSAGE TEXT. Callers
--   (apps/admin/.../menuScan.ts) match on the MESSAGE TEXT to translate to
--   user-facing strings. PostgreSQL transaction wrapping ensures any partial
--   state is rolled back when an exception fires.
--
-- Reverse: 144_REVERSE_ONLY_admin_menu_scan_and_modifier_rpcs.sql drops both
-- functions. Reverting requires switching the admin app back to the inline
-- multi-pass insert flow (commit prior to Phase 4.2).
...
CREATE OR REPLACE FUNCTION public.admin_replace_dish_modifiers(
  p_dish_id  uuid,
  p_groups   jsonb
)
RETURNS jsonb
...
DECLARE
  ...
  v_modifier_options_count integer := 0;
BEGIN
  -- Validate dish exists; capture restaurant_id for option_groups insert.
  SELECT restaurant_id INTO v_restaurant_id
    FROM public.dishes
   WHERE id = p_dish_id
   FOR UPDATE;
  IF NOT FOUND THEN
    RAISE EXCEPTION 'DISH_NOT_FOUND' USING ERRCODE = 'P0002';
  END IF;

  -- Payload shape sanity check (callers should validate; defensive only).
  IF p_groups IS NULL OR jsonb_typeof(p_groups) <> 'array' THEN
    RAISE EXCEPTION 'INVALID_GROUPS_PAYLOAD' USING ERRCODE = '22023';
  END IF;

  -- Delete existing groups (FK ON DELETE CASCADE handles options).
  DELETE FROM public.option_groups WHERE dish_id = p_dish_id;
  ...
  FOR v_group IN SELECT * FROM jsonb_array_elements(p_groups) LOOP
    INSERT INTO public.option_groups (
      ...
      v_restaurant_id, p_dish_id, v_group->>'name',
      COALESCE(v_group->>'selection_type', 'single'),
      COALESCE((v_group->>'min_selections')::integer, 0),
      COALESCE((v_group->>'max_selections')::integer, 1),
      v_group_order, true,
  ...
  RETURN jsonb_build_object(
    'group_count', v_modifier_groups_count,
    'option_count', v_modifier_options_count
  );
END;
$fn$;

REVOKE EXECUTE ON FUNCTION public.admin_replace_dish_modifiers(uuid, jsonb) FROM PUBLIC;
GRANT  EXECUTE ON FUNCTION public.admin_replace_dish_modifiers(uuid, jsonb) TO service_role;

COMMENT ON FUNCTION public.admin_replace_dish_modifiers(uuid, jsonb) IS
  'Phase 4.2 — atomically replace a dish''s full modifier_groups set. '
