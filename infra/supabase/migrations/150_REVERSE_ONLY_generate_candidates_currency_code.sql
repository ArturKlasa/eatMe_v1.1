-- 150_REVERSE_ONLY_generate_candidates_currency_code.sql
-- Reverses 150_generate_candidates_currency_code.sql by restoring the
-- generate_candidates function body from migration 142 (no
-- restaurant_currency_code column).
--
-- The forward migration's only behavioural change was adding one return
-- column (r.currency_code AS restaurant_currency_code). This reverse drops
-- the function and recreates it with migration 142's exact signature + body.
