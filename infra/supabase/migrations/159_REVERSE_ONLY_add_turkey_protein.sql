-- 159_REVERSE_ONLY_add_turkey_protein.sql
-- Reverse of 159_add_turkey_protein.sql. Apply ONLY to roll 159 back.
--
-- 1. Reclassifies existing 'turkey' rows to 'other_meat' (mirror of migration
--    131's duck handling): families lose 'poultry', canonical names rewrite.
-- 2. Restores the 11-value CHECK constraints.
-- 3. Restores the migration-155 bodies of generate_candidates /
--    get_group_candidates (no turkey arms).
-- 4. Drops the compute_dish_protein_families trigger + function (they did not
--    exist before 159).
--
-- NOTE: the protein_families backfill from 159 §6 is deliberately NOT
-- reversed — it fixed rows that were always supposed to be populated;
-- emptying them again would re-break the vegetarian filter.

BEGIN;

-- ── 1. Reclassify turkey rows ────────────────────────────────────────────────
...
WHERE primary_protein = 'turkey';

UPDATE public.user_preferences
SET primary_protein = 'other_meat'
WHERE primary_protein = 'turkey';

-- ── 2. Restore 11-value CHECK constraints ────────────────────────────────────
...
-- ── 3. Restore migration-155 RPC bodies ──────────────────────────────────────
...
-- ── 4. Drop the trigger + function added by 159 ──────────────────────────────

DROP TRIGGER IF EXISTS trg_compute_dish_protein_families ON public.dishes;
DROP FUNCTION IF EXISTS public.compute_dish_protein_families();
