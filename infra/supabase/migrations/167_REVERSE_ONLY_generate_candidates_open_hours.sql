-- ══════════════════════════════════════════════════════════════════════════════
-- REVERSE of migration 167 — restore the migration-163 generate_candidates.
--
-- Drops the open_hours/timezone/country_code-returning version and recreates the
-- 28-column form VERBATIM from migration 163 §1. The DROP matches on arg signature
-- (unchanged between 163 and 167), so it removes the 167 function cleanly.
--
-- Run this AFTER redeploying the prior feed function build (the one that issues the
-- second open_hours query) — otherwise the live function expects columns this
-- version no longer returns.
-- ══════════════════════════════════════════════════════════════════════════════
