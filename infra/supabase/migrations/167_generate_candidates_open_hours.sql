-- ══════════════════════════════════════════════════════════════════════════════
-- Migration 167: generate_candidates returns open_hours / timezone / country_code
--
-- §S3 (docs/findings/mobile-performance-audit.md): the feed Edge Function made a
-- second, serial query to fetch open_hours/timezone/country_code AFTER
-- generate_candidates returned, solely to compute open-now. That data is already
-- on the restaurants row the RPC joins (r.*). Return it directly so the Edge
-- Function builds the open-now map in memory and drops the round-trip.
--
-- Body is VERBATIM from migration 163 §1 with exactly two additions:
--   * RETURNS TABLE  += open_hours JSONB, timezone TEXT, country_code TEXT
--   * SELECT         += r.open_hours, r.timezone, r.country_code
-- The arg signature is unchanged; only the return shape changes → DROP + CREATE.
--
-- ⚠ DEPLOY ORDERING: apply THIS MIGRATION BEFORE deploying the new feed function.
-- The updated function reads these columns off candidate rows; if it ships first
-- they are undefined → every restaurant reads as closed → empty feed. The old
-- live function ignores the extra columns, so migration-first is safe.
-- ══════════════════════════════════════════════════════════════════════════════
