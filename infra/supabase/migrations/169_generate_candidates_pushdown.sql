-- ══════════════════════════════════════════════════════════════════════════════
-- Migration 169: generate_candidates — modifier-JSON push-down (§S9)
--
-- §S9 (docs/findings/mobile-performance-audit.md): generate_candidates exceeds
-- Postgres' 8s statement_timeout (error 57014) past ~5km radius on the full
-- published-dish set (~8.4k). Root cause is the `dish_modifiers` CTE in migration
-- 167: it builds the full modifier_groups JSON (a jsonb_agg with a correlated
-- subquery into `options`) for EVERY dish that has active option_groups in the
-- whole DB — with NO radius filter and NO LIMIT — even though the query returns at
-- most p_limit (200) rows. Whether that whole-table aggregate lands on the hot path
-- is a *planner choice* driven by stats: with fresh stats it nested-loops modifiers
-- for the few in-range dishes (~1s); with stale stats it hash-aggregates modifiers
-- across all option_groups and tips over 8s. That is why a one-off ANALYZE "fixed"
-- it and why it regressed two days later.
--
-- This migration removes the planner's freedom to pick the bad plan by making the
-- modifier work bounded-by-construction:
--   (1) A MATERIALIZED `candidates` CTE applies every cheap filter (radius, diet,
--       time/day, …) + ORDER BY + LIMIT with NO modifier work, carrying the sort
--       keys as columns. Cost ∝ in-range dishes (index-assisted), never the whole
--       option_groups table.
--   (2) A LATERAL builds modifier_groups JSON + option_proteins for ONLY the ≤200
--       survivors (uses prod indexes option_groups_dish_id_idx /
--       options_option_group_id_idx, verified present — audit §S4).
--   (3) The dish-level `required_groups_safe` filter (the only WHERE use of the old
--       CTE) is rewritten inline as a NOT EXISTS that runs only when a diet filter
--       is active. Logic is identical — see the boundary-case note on the predicate.
--
-- `#variable_conflict use_column`: the candidates-CTE column aliases (vector_distance,
-- popularity_score, distance_m) share names with this function's RETURNS TABLE OUT
-- parameters. The directive forces bare references in ORDER BY to resolve to the
-- columns; without it PL/pgSQL could resolve them to the (NULL) OUT-param variables
-- and silently mis-sort. All other column refs are schema-qualified, and every
-- parameter is p_-prefixed, so the directive affects nothing else.
--
-- The arg signature AND the 32-column return shape are UNCHANGED from migration 167,
-- so this is a CREATE OR REPLACE (no DROP) and is backward/forward compatible with
-- either feed build — no deploy-ordering constraint.
-- ══════════════════════════════════════════════════════════════════════════════
